...
interface MapEmitter<K2,V2> { void mapEmit(K2 k2, V2 v2); }
interface ReduceEmitter<K3,V3> { void reduceEmit(K3 k3, V3 v3); }
interface MapFunction<K1,V1,K2,V2> { public void map(K1 key, V1 value, MapEmitter<K2,V2> emitter); }
interface ReduceFunction<K2,V2,K3,V3> { public void reduce(K2 key, List<V2> values, ReduceEmitter<K3,V3> emitter); }
...
public class MapReduce<K1,V1,K2,V2,K3,V3> implements MapEmitter<K2,V2>, ReduceEmitter<K3,V3> {
	...
	public Map<K3,V3> mapReduce(Map<K1,V1> input) {
		...
		for(Thread w : reduceWorkers) try { w.join(); } catch(InterruptedException ie) {}
		return finalResult;
	}
	...
	public static void main(String[] args) {
		...
		MapFunction<Integer,Integer,Integer,Integer> map = 
			(key, value, emitter) -> {
		...
		MapReduce<Integer,Integer,Integer,Integer,Integer,Integer> hej = new MapReduce(map, reduce);
